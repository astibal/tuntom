#!/usr/bin/env python3
"""Live stats and continuous sampling without filesystem export."""
import os
import socket
import subprocess
import sys
...
def run_case(tuntom, switch, ctl, missing_file):
    processes = []
    with tempfile.TemporaryDirectory(prefix="tuntom-stats-socket.") as directory:
        ...
        try:
            ...
            assert "stats_enabled" not in snapshot(ctl, server_control)
            assert "stats_enabled" not in snapshot(ctl, client_control)
            ...
            if not missing_file:
                ...
                wait_until(lambda: float(snapshot(ctl, server_control)["udp_rx_bps_5s"]) > 0,
                           "throughput stopped without file export")
                assert float(snapshot(ctl, server_control)["udp_rx_pps_5s"]) > 0
                before = snapshot(ctl, client_control)
                for _ in range(20):
                    exchange(app, payload, expected, processes)
                after = snapshot(ctl, client_control)
                assert int(after["udp_tx_packets"]) > int(before["udp_tx_packets"])
                assert int(after["throughput_window_buckets"]) >= int(before["throughput_window_buckets"])
                assert float(after["udp_tx_bps_1m"]) > 0
                time.sleep(1.1)
                assert file_state(client_stats) == original_file
                assert not any(".tmp." in name for name in os.listdir(directory))
        finally:
...
if __name__ == "__main__":
    for missing_file in (False, True):
        run_case(*sys.argv[1:4], missing_file)
    print("PASS: stats without files, no file export, continued sampling/rates")
