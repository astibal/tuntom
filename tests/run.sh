...
if command -v python3 >/dev/null 2>&1; then
    ...
    "${CXX:-g++}" -std=c++17 -pthread -O2 -Wall -Wextra -Wconversion -pedantic \
        "${tests_dir}/../src/main.cpp" "${tests_dir}/adapter_tun_fixture.cpp" \
        -Wl,--wrap=open -Wl,--wrap=ioctl -o "${build_dir}/tunnel_tun_fixture"
    "${CXX:-g++}" -std=c++17 -shared -fPIC -O2 -Wall -Wextra -Wconversion -pedantic \
        "${tests_dir}/endpoint_faults.cpp" -ldl -o "${build_dir}/endpoint_faults.so"
    python3 "${tests_dir}/endpoint_recovery_test.py" \
        "${build_dir}/tuntom" "${build_dir}/tuntom-switch" \
        "${build_dir}/adapter_reconnect_fixture" "${build_dir}/tunnel_tun_fixture" \
        "${build_dir}/endpoint_faults.so"
...
"${CXX:-g++}" -std=c++17 -O2 -Wall -Wextra -Wconversion -pedantic \
    "${tests_dir}/endpoint_recovery_test.cpp" "${tests_dir}/adapter_tun_fixture.cpp" \
    -Wl,--wrap=open -Wl,--wrap=ioctl -Wl,--wrap=socket -Wl,--wrap=bind \
    -Wl,--wrap=setsockopt -Wl,--wrap=getsockopt -Wl,--wrap=getaddrinfo \
    -Wl,--wrap=read -Wl,--wrap=write -o "${build_dir}/endpoint_recovery_test"
"${build_dir}/endpoint_recovery_test"
