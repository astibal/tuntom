...
class Endpoint {
    ...
    void close_upstream() { if (upstream_ >= 0) ::close(upstream_); upstream_ = -1; registered_ = false; }
public:
    ...
    void descriptors(std::vector<pollfd>& out) const {
        ...
        if (upstream_ >= 0) out.push_back({upstream_, static_cast<short>(registered_ ? POLLIN : POLLOUT), 0});
        for (const auto& c : clients_) out.push_back({c->fd, static_cast<short>(c->active ? POLLIN : c->handshake.events()), 0});
    }
    void receive(const std::uint8_t* data, std::size_t size) {
        ...
        if (!listening()) {
            if (registered_ && (v.type == Type::snapshot || v.type == Type::data))
                (void)::send(upstream_,data,size,MSG_DONTWAIT|MSG_NOSIGNAL);
            return;
        }
        ...
        for (auto& c : clients_) if (c->active && c->channel == v.channel) {
            (void)c->handshake.transport->send(c->fd, {v.payload, v.size, nullptr, 0}); return;
        }
    }
    void step(const Send& send) {
        ...
        for(auto it=clients_.begin();it!=clients_.end();) {
            ...
            if(dead) { if(c.active) changed(); it=clients_.erase(it); } else ++it;
        }
