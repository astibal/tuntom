...
inline bool ip6_extension(std::uint8_t protocol) {
    return protocol == 0 or protocol == 43 or protocol == 44 or
           protocol == 51 or protocol == 60;
}
...
inline bool parse_ip_flow(
    ...
    if (version == 4) {
        ...
        for (std::size_t count = 0; count < 8; ++count) {
            if (protocol == 44) {
                ...
                if ((fragment & 0xfff8U) != 0) {
                    // A later fragment cannot reveal extension headers after
                    // its fragment header, but may name the transport directly.
                    parsed.protocol = protocol;
                    parsed.has_protocol = not ip6_extension(protocol);
                    return true;
                }
        ...
        if (ip6_extension(protocol)) return false; // Bounded extension traversal.
    } else {
        return false;
    }
