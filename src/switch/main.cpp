    ...
    try {
        ...
        while (not stop_requested) {
            ...
            if (descriptors[0].revents & POLLIN) {
                ...
                if (accepted >= 0) connections.push_back({accepted, {}});
            }

            const std::size_t polled_connections = descriptors.size() - 1;
            for (std::size_t index = 0; index < polled_connections; ++index) {
                auto& connection = connections[index];
                const auto events = descriptors[index + 1].revents;
                ...
                if (route != routes.end()) {
                    ...
                    send_frame(connection.fd, output);
                }
            }
