...
enum class Kind : std::uint8_t { put=1, status=2, reply=3, finish=4, confirmed=5, discover=6, found=7, alt_path=8, route_error=9 };
...
inline bool fields_valid(const Frame& f) {
    ...
    if (kind < 1 || kind > 9 || !f.remaining || f.remaining > max_hops || f.request == Id{} || f.origin == Id{} || f.command.size() > 256) return false;
    if (kind <= 5) {
...
inline std::vector<std::uint8_t> encode(const Frame& f) {
    ...
    const auto size = header_size + destination.size() + reply.size() + f.command.size() + f.data.size();
...
inline bool decode(const std::uint8_t* p, std::size_t n, Frame& output) {
    if (n < header_size || n > max_frame || p[0] != 2 || p[50] || p[51]) return false;
    ...
    const std::size_t command=load_be16(p+44), destination=load_be16(p+46), reply=load_be16(p+48);
    if (command > 256 || header_size+command+destination+reply > n) return false;
    if (!decode_path(p+header_size,destination,f.destination) || !decode_path(p+header_size+destination,reply,f.reply_path)) return false;
    const auto at=header_size+destination+reply;
...
inline Frame wrap(const remote_control::Frame& f, const Id& origin, Path path) {
    Frame out; out.kind=static_cast<Kind>(f.kind); out.state=static_cast<std::uint8_t>(f.state);
    out.request=f.id; out.origin=origin; out.destination=std::move(path);
    out.offset=f.offset; out.total=f.total; out.command=f.command; out.data=f.data; return out;
}
inline remote_control::Frame unwrap(const Frame& f) {
    if (static_cast<unsigned>(f.kind)>5) throw std::runtime_error("not a control transaction");
    remote_control::Frame out; out.kind=static_cast<remote_control::Kind>(f.kind); out.state=static_cast<remote_control::State>(f.state);
    out.id=f.request; out.offset=f.offset; out.total=f.total; out.command=f.command; out.data=f.data; return out;
}
